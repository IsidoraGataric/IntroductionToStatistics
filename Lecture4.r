...
# SD (Standard deviation). This is a measure that tells us how many individual observations deviate from AS, this measure is squared, then all the values obtained are summed and eventually divided by n-1 (Bassel's correction).
sd(town1)
sd(town2)
...
# Variance. This measure counts over SD (and represents normalized SD, so we can compare it with other data sets).
sd(town1)/mean(town1)
